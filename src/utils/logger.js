...
export class Logger {
  ...
  #formatDate(date) {
    ...
    const month = pad(date.getMonth() + 1); // +1 porque getMonth() va de 0-11
  ...
  #print({ level, color, writer, message, meta = {} }) {
    ...
    writer(
      `${COLORS.magenta}[${timestamp}] ${levelText} ${contextText} ${messageText}`,
    );
    if (Object.keys(meta).length > 0) {
      console.log(
        `${COLORS.gray}${util.inspect(meta, { depth: 4, colors: true, compact: false })}${COLORS.reset}`,
      );
    }
  }
